...
function minimumPasses(m, w, p, n) {
    ...
    let pass = 0, min = Math.ceil(n / ( m * w)), candies = 0;
    
     while(pass < min) {
         let dp = Math.ceil((p - candies) / ( m * w));
         pass += dp;
         candies += m * w * dp;
         ...
         if (Math.floor(candies / p) >= Math.abs( m - w)) {
             ...
             m > w ? w = m : m = w;
             let upgrades = Math.floor(candies / p);
             if (upgrades > 0) {
                 if (upgrades % 2 === 0) {
                     m = w = m + (upgrades / 2);
                 } else {
         ...
         let mp = Math.ceil((n - candies) / ( m * w));
         
         if (min > mp + pass) min = mp + pass;
     }
     return min;
